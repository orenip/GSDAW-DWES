    ...
    public function nuevo() {
        ...
        if (isset($_REQUEST['submit'])) {
            ...
            if (!isset($_REQUEST['nombre']) || empty($_REQUEST['nombre']))
             ...
             // Comprobamos si se ha recibido el salario
             if (!isset($_REQUEST['salario']) || empty($_REQUEST['salario']))
             $errores['salario'] = "* salario: debes indicar un salario.";

            // Si no hay errores actualizamos en la BD
            if (empty($errores)) {
                $empleado->setCodigo($_REQUEST['codigo']);
                $empleado->setNombre($_REQUEST['nombre']);
                $empleado->setSalario($_REQUEST['salario']);
                $empleado->save();
