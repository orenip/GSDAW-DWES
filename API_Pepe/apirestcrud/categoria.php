 ...
 *	La clase "categoria.class.php" es la clase del modelo, que representa una categoría de productos
*/
...
switch ($_SERVER['REQUEST_METHOD']) {
	...
	case 'POST':
		...
		if(!isset($params)){
			$response = array(
				'result' => 'error',
				'details' => 'Error en la solicitud, no se han recibido parámetros'
			);
	...
	case 'PUT':
		...
		if(!isset($params) || !isset($_GET['cat_id']) || empty($_GET['cat_id'])){
			$response = array(
				'result' => 'error',
		...
		$categoria->update($_GET['cat_id'], $params);
