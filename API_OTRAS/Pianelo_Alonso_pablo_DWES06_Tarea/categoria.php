...
switch ($_SERVER['REQUEST_METHOD']) {//Se mira el tipo de petición que ha llegado a la API: GET, POST, PUT o DELETE
	...
	case 'GET':
		...
		if(isset($params["articulos"])) {
			$data = $categoria->articulosDeCategoria($params["cat_id"]);
		} else {
			$data = $categoria->get($params);
		}
		 //Esta línea de código en PHP está llamando al método get en el objeto $player y pasando el array $params como argumento. El resultado de esta llamada al método se almacena en la variable $players.
...
y convertirlos en un array de PHP que se puede manipular más fácilmente en el código.*/
	...
	case 'PUT':
		...
		$categoria->updatePut($_GET['cat_id'], $params);//Se llama al método update() del modelo, pasándole como parámetros el id del jugador y el array $params
		...
		case 'PATCH':
			...
			$categoria->updatePatch($_GET['cat_id'], $params);//Se llama al método update() del modelo, pasándole como parámetros el id del jugador y el array $params
	...
	case 'DELETE':
		...
		$resp = $categoria->delete($_GET['cat_id']);//Se llama al método delete() del modelo, pasándole como parámetro el id del jugador
