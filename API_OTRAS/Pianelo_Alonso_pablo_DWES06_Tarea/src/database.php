...
class Database
{
	...
	public function __construct(){
		$this->connection = new mysqli('localhost', 'root', '', 'almacen', '3306');
//Se crea una conexión a la base de datos
	 ...
	 */
	public function updateDB($table,$pk, $id, $data)//El parámetro $data es un array que contiene los campos y valores que se quieren actualizar
	{	
		...
		$query .= ' WHERE '.$pk.'= '.$id;//Añade el id del registro que se quiere actualizar a la consulta
	 ...
	 * Hay que indicar el registro mediante un campo que sea clave primaria y que debe llamarse "id"
	 */
	public function deleteDB($table,$pk, $id)//El parámetro $id es el id del registro que se quiere eliminar
	{
		$query = "DELETE FROM $table WHERE $pk = $id";//Crea la consulta
		$this->connection->query($query);//Ejecuta la consulta
