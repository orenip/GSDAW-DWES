...
class Categoria extends Database
{
	...
	{
		$articulos = parent::getDB("articulo", array("art_categoria" => $id));
		if(count($articulos)) {
			$response = array(
				'result' => 'error',
				'details' => 'No se ha podido borrar la categoría.'
			);
			Response::result(400, $response);
			exit;
		}
		$affected_rows = parent::deleteDB($this->table,$this->pk, $id);
		//Llama al método deleteDB de la clase Database (parent de player, que player extends Database), pasándole como parámetros la tabla y el id del registro que se quiere eliminar
	...
	public function articulosDeCategoria($id)//Recibe como parámetro el id del registro que se quiere eliminar
	{
		return $articulos = parent::getDB("articulo", array("art_categoria" => $id));
	}
}
