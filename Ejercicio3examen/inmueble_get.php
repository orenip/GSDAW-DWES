...
<body>
    ...
    if (is_array($array_datos) && count($array_datos) > 0) {
        ...
        foreach ($array_datos as $inmueble) {
            if (is_array($inmueble) && isset($inmueble['id']) && isset($inmueble['inm_referencia'])) {
                if (
                    ($filter_cat_id === null || $inmueble['id'] == $filter_cat_id) &&
                    ($filter_cat_nombre === null || stripos($inmueble['inm_referencia'], $filter_cat_nombre) !== false)
                ) {
                    echo "<h2>Inmueble con id= " . $inmueble['id'] . ": " . $inmueble['inm_referencia'] . "</h2>";
                }
            } else {
                echo "<p>Invalido inmueble formato</p>";
            }
        }
    } else {
        echo "<p>No se encontraron inmueble.</p>";
    }
    ?>
